...
- **Upstream/downstream:** Cloud (Kayvan) consumes the MQTT stats for the AgCloud dashboard. Embedded consumes stdout logs via journald or equivalent. Documentation captures the log format.
- **Definition of done:** Pipeline emits JSON logs on stdout with fields (timestamp, level, event, task, robot_id, extra). MQTT stats message published every 60s on `robogreeno/stats/<robot_id>`. Kayvan confirms the stats fit his dashboard needs.
- **Demo:** Run pipeline for 5 minutes; show `mosquitto_sub` receiving stats messages; grep logs to trace a specific frame end-to-end.
- **Subtasks:**
