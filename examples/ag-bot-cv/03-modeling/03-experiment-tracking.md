...
Anyone else on the team running experiments needs to find and compare runs.

## Definition of Done

All 4 students' training runs appear in the shared W&B project. Every run has config, train/val metrics per epoch, and a link back to the git commit.

## Demo

Show two runs side-by-side in W&B; explain the delta.
