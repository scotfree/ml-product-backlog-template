...
- Model analysis (03-error-analysis) informs what to try next.
- Deployment (03-benchmark) informs whether YOLO11s is even viable on Pi 3B CPU.

## Definition of Done

`docs/experiments.md` with one line per run: what changed, mAP delta, notes. Best-so-far checkpoint per task promoted to `models/<task>_best.pt`.

## Demo

Show the experiment log; call out the best change and one thing that didn't help.
...
Ongoing across Sprint 3 — days of wall-clock, mostly training time.
