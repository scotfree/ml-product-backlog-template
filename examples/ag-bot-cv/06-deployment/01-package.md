...
- **Upstream/downstream:** Embedded (Pavan) consumes the wheel and decides how to supervise it on the Pi (systemd is his call, not ours). Cloud consumes the resulting MQTT stream.
- **Definition of done:** `pip install dist/robogreeno_detection-*.whl` on a clean Python 3.11 env installs code + models. `python -m robogreeno_detection --config configs/pi.yaml` runs the pipeline. `SIGTERM` triggers graceful shutdown (drain + disconnect + exit 0).
- **Demo:** Install the wheel in a fresh venv on a laptop; run it against local `mosquitto`; kill with SIGTERM; show clean shutdown in logs.
- **Subtasks:**
