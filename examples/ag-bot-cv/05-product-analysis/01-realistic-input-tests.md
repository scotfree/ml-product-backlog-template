...
- **Upstream/downstream:** Deployment (all cards) consumes results as a go/no-go signal. Feeds back to Modeling if a domain gap becomes obvious.
- **Definition of done:** `tests/realistic/` directory with ~30 curated real-world inputs and expected outcomes. `pytest tests/realistic/` runs the pipeline on each and produces a pass/fail report.
- **Demo:** Run the suite; walk through one clear pass (obvious ripe tomato) and one clear failure (bad lighting misclassified).
- **Subtasks:**
