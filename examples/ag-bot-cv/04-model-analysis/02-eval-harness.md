...
- Everyone comparing models depends on this.
- Product analysis (Card 01-realistic-input-tests) may extend it with realistic-input evaluation.

## Definition of Done

`evaluate.py` runs deterministically (seeded). Same model + same command → identical numbers. Reports committed to `reports/` per model version.

## Demo

Run harness on `fruit_yolo11n_v0` and `fruit_yolo11n_v3` side-by-side; show the comparison.
